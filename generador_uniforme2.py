...
class GeneradorUniforme2App:
    def __init__(self, root):
        ...
        style.map("Treeview.Heading", background=[('active', "#F48FB1")])
    ...
    def mostrar_histograma(self, r_values):
        ...
        ax.hist(r_values, bins=10, alpha=0.7, color='#F8BBD0', edgecolor='black', linewidth=1.5)  # Rosa suave
        kde = gaussian_kde(r_values)
        x = np.linspace(0, 1, 100)
        ax.plot(x, kde(x), color='black', linewidth=2, label='Densidad Observada (KDE)')
        ax.axhline(y=0.5, color='black', linestyle='--', linewidth=2, label='Frecuencia Esperada (0.5)')
